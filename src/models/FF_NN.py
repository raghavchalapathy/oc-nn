...
class FF_NN:
    ...
    def fit(self,trainX,trainY,nEpochs,IMG_HGT,IMG_WDT,IMG_DEPTH,nClass):
        ...
        trainX= FF_NN.image_to_feature_vector(trainX, IMG_HGT, IMG_WDT)

        model = FF_NN.build(width=IMG_HGT, height=IMG_WDT, depth=IMG_DEPTH, classes=nClass)
        ...
        plt.title("FF_NN Training Loss and Accuracy on 1's / 7's")
        plt.xlabel("Epoch #")
